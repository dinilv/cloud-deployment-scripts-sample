...
file_array="${file_array} /var/tracker/api/postback/nohup.out"
file_array="${file_array} /var/tracker/services/postback/nohup.out"
file_array="${file_array} /var/tracker/services/postbackoffermedia/nohup.out"
file_array="${file_array} /var/tracker/broker/exception/nohup.out"
file_array="${file_array} /var/tracker/subscriber/filtered/nohup.out"
file_array="${file_array} /var/tracker/subscriber/postback/nohup.out"
file_array="${file_array} /var/tracker/subscriber/delayed/nohup.out"
file_array="${file_array} /var/tracker/subscriber/postback_ping/nohup.out"
...
curl -F "file=@scanner.txt;" -F "server_type=tracker_postback" -F "public_ip=$public_ip" -F "bind_ip=$bind_ip" -F "event=error_scanner" -F "module=error_scanner" -F "env=prod" -F "file_name=scanner.txt" -F "host=$HOST" http://0.0.0.0:8080/cluster/node/health
