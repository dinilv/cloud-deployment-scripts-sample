...
curl -F "file=@memory.txt;" -F "server_type=tracker_db_cluster" -F "public_ip=$public_ip" -F "bind_ip=$bind_ip" -F "event=memory_check" -F "module=db" -F "env=prod" -F "file_name=memory.txt" -F "host=$HOST" http://0.0.0.0:8080/cluster/node/health
