...
curl -F "file=@monitor.txt;" -F "server_type=tracker_click" -F "public_ip=$public_ip" -F "bind_ip=$bind_ip" -F "event=disk_check" -F "module=db" -F "env=prod" -F "file_name=monitor.txt" -F "host=$HOST" http://0.0.0.0:8080/cluster/node/health
