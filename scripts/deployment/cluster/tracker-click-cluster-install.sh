...
VERSION=$(curl -s http://0.0.0.0:8080/deployment/version?server_type=tracker_click&host=$HOST)
DEPLOY_VERSION="v1"
if [ -z "$VERSION" ]; then
...
mkdir -p tracker/subscriber/click tracker/subscriber/rotated
...
#---CLICK
cd $HOME
cd tracker/subscriber/click
wget https://storage.g.tracker/$DEPLOY_VERSION/$DEPLOY_VERSION.subscriber.tracker.click
chmod 777 *
#----ROTATED
cd $HOME
cd tracker/subscriber/rotated
wget https://storage.g.tracker/$DEPLOY_VERSION/$DEPLOY_VERSION.subscriber.tracker.rotated
chmod 777 *
#---API
#---CLICK
cd $HOME
...
cd $HOME
cd tracker/broker/exception
nohup ./$DEPLOY_VERSION.broker.tracker.exception --selector=cache --broker=googlepubsub --client_pool_size=1000 > nohup.out &
#start subscribers
cd $HOME
cd tracker/subscriber/click
nohup ./$DEPLOY_VERSION.subscriber.tracker.click 6003 > nohup.out &
cd $HOME
cd tracker/subscriber/rotated
nohup ./$DEPLOY_VERSION.subscriber.tracker.rotated 6004 > nohup.out &
#click api
cd $HOME
cd tracker/api/click
nohup ./$DEPLOY_VERSION.api.tracker.click --selector=cache --broker=googlepubsub --client_pool_size=1000 > nohup.out &
nohup ./$DEPLOY_VERSION.api.tracker.click --selector=cache --broker=googlepubsub --client_pool_size=1000 > nohup.out &
nohup ./$DEPLOY_VERSION.api.tracker.click --selector=cache --broker=googlepubsub --client_pool_size=1000 > nohup.out &
